...
def entity_recognition(text):
    ...
    for ent in doc.ents:
        if ent.label_ in ['DATE', 'TIME', 'PERCENT', 'MONEY', 'QUANTITY', 'ORDINAL', 'CARDINAL']:
            continue
        entities.append([ent[0], ent.start_char, ent.end_char])
...
def candidate_selection(entity, lsh_k, anchor_k):
    """
...
def fake_candidate_selection_and_disambiguation(ner_data):
    import pickle
    import numpy as np
    with open('../data/mod_anchor_candidates.pkl', 'rb') as f:
        candidates = pickle.load(f)
    
    # fake candidate selection and disambiguation. Just pick first one from set
    try:
#         ner_data['predicted_wikidata'] = ner_data.apply(lambda i: np.random.choice(candidates[i.entity]), axis=1)
        ner_data['predicted_wikidata'] = ner_data.apply(lambda i: candidate_selection(i.entity, 10, 10)[0], axis=1)
...
def predict_entity(raw_data):
    ...
    pred_data = fake_candidate_selection_and_disambiguation(cleaned_ner_data)
